...
module.exports = {
  ...
  options: [
    ...
    { name: 'TYPES',  short: 'f', long: 'types',  type: 'string', description: 'event types' },
    ...
    { name: 'FROM',   short: 'b', long: 'from',   type: 'string', description: 'start from this event' },
  ...
  run: async (toolbox, args) => {
    ...
    const filters = { types: args.TYPES, after: args.AFTER, until: args.UNTIL, from: args.FROM, to: args.TO }
    if (args.NS) filters.ns = account.db(args.DB).ns(args.NS)
    if (args.STREAM) filters.stream = (filters.ns || account.db(args.DB)).stream(args.STREAM)
    if (args.EVENT) {
      filters.from = args.EVENT
      filters.limit = 1
    }
    const promise = account.db(args.DB).events(filters).each(event => {
      log()
